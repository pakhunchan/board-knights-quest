...
{
    /// <summary>
    /// Editor utility that builds the Fractions Demo scene from scratch.
    /// Menu: Board of Education > Build Fractions Demo Scene
    /// </summary>
    public static class FractionsDemoSceneBuilder
    {
        ...
        public static void BuildFractionsDemoScene()
        {
            ...
            // ── Title ──
            var titleGo = CreateText(canvasGo.transform, "Title", "FRACTIONS DEMO",
                48, TextAlignmentOptions.Center, Color.white);
            SetAnchored(titleGo, new Vector2(0.1f, 0.85f), new Vector2(0.9f, 0.95f));

            // ── EquationRow ──
            var equationRowGo = CreateUIElement("EquationRow", canvasGo.transform);
            var equationRowRect = equationRowGo.GetComponent<RectTransform>();
            equationRowRect.anchorMin = new Vector2(0.05f, 0.35f);
            equationRowRect.anchorMax = new Vector2(0.95f, 0.65f);
            equationRowRect.offsetMin = Vector2.zero;
            equationRowRect.offsetMax = Vector2.zero;
            ...
            // ── Save Scene ──
            string scenePath = "Assets/Scenes/FractionsDemo.unity";
            if (!AssetDatabase.IsValidFolder("Assets/Scenes"))
                AssetDatabase.CreateFolder("Assets", "Scenes");

            UnityEditor.SceneManagement.EditorSceneManager.SaveScene(scene, scenePath);
            Debug.Log($"[FractionsDemoSceneBuilder] Fractions Demo scene built and saved to {scenePath}");
        }
        ...
        private static void SetRef(SerializedObject so, string fieldName, Object value)
        {
            ...
            if (prop != null)
                ...
                Debug.LogWarning($"[FractionsDemoSceneBuilder] Could not find field '{fieldName}' on {so.targetObject.GetType().Name}");
        }
