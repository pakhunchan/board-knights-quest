...
{
    /// <summary>
    /// Editor utility that builds the MVP1 scene (full lesson + practice).
    /// Menu: Board of Education > Build MVP1 Scene
    /// </summary>
    public static class MVP1SceneBuilder
    {
        [MenuItem("Knight's Quest: Math Adventures/Build MVP1 Scene")]
        public static void BuildMVP1Scene()
        {
            ...
            var manager = gameCoreGo.AddComponent<MVP1Manager>();
            ...
            // ── Save Scene ──
            string scenePath = "Assets/Scenes/MVP1.unity";
            if (!AssetDatabase.IsValidFolder("Assets/Scenes"))
                AssetDatabase.CreateFolder("Assets", "Scenes");

            UnityEditor.SceneManagement.EditorSceneManager.SaveScene(scene, scenePath);
            Debug.Log($"[MVP1SceneBuilder] MVP1 scene built and saved to {scenePath}");
        }
        ...
        private static void SetRef(SerializedObject so, string fieldName, Object value)
        {
            ...
            if (prop != null)
                ...
                Debug.LogWarning($"[MVP1SceneBuilder] Could not find field '{fieldName}' on {so.targetObject.GetType().Name}");
        }
